...
test('shows story rings and the "Today in space" banner', async ({ page }) => {
  ...
  await expect(banner.getByRole('group', { name: 'Choose slide' }).getByRole('button')).toHaveCount(
    7,
  );
});
...
test('the banner can be navigated with the keyboard', async ({ page }) => {
  ...
  await expect(banner.locator('.ui-carousel__slide--active')).toHaveAttribute(
    'aria-label',
    '2 of 7',
  );
...
test('a banner slide opens its story', async ({ page }) => {
  ...
  await page.keyboard.press('ArrowRight');
  await banner
    .locator('.ui-carousel__slide--active')
    .getByRole('link', { name: 'Open the story' })
    .click();
