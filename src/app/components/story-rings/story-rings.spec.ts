...
describe('StoryRings', () => {
  ...
  function render(transitionsEnabled = true) {
    ...
    const links = (): HTMLAnchorElement[] => [...element.querySelectorAll<HTMLAnchorElement>('a.ring')];
  ...
  it('moves fully seen groups to the end, in grey', () => {
    ...
    expect(links().map((link) => link.textContent.trim().split(',')[0])).toEqual(['Latest', 'Galaxies', 'Stars']);
