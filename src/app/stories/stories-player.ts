...
export type StoryPauseReason = 'user' | 'hold' | 'hidden' | 'loading' | 'caption' | 'video' | 'moving' | 'dismiss';
...
export class StoriesPlayer {
  ...
  public readonly groupId = input<string>('latest');
  ...
  public readonly cursor = computed<StoryCursor>(() => {
    const group = this.groupIndex();
    return {group, story: this.storyByGroup().get(group) ?? 0};
  });

  public readonly currentGroup = computed(() => this.groups()[this.cursor().group] ?? null);
  public readonly current = computed(() => this.currentGroup()?.entries[this.cursor().story] ?? null);
  ...
  constructor() {
    ...
    this.engine.subscribe((state) => {
      ...
      if (state.phase === 'animating' && state.position !== previous.position) this.scheduleSettle();
    });
  ...
  public goToGroup(group: number): void {
  ...
  public isPausedBy(reason: StoryPauseReason): boolean {
  ...
  public async share(): Promise<void> {
    ...
    const url = new URL(this.location.prepareExternalUrl(this.router.url), this.document.location.origin).toString();
